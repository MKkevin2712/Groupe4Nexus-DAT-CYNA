...
Cette section détaille les procédures nécessaires pour configurer, exécuter et tester les environnements de développement de l'écosystème CYNA Defense (Backend, Frontend Web et Application Mobile).

### 1. Prérequis Systèmes

Avant toute manipulation, les outils suivants doivent être installés sur la machine de développement :

-   Node.js (Version LTS recommandée)
-   npm (Gestionnaire de paquets)
-   Git
-   Pour le mobile : L'application Expo Go sur smartphone (iOS/Android), ou un émulateur local (Android Studio / Xcode).
...
> ⚠️ **Règle de sécurité** : Les clés secrètes (`SERVICE_KEY`, `STRIPE_SECRET`, mots de passe) ne doivent jamais être exposées dans le dossier frontend.
...
-   **Terminal 1 (Backend)** : `cd cyna-backend && npm run start:dev` (Écoute sur le port 3000)
-   **Terminal 2 (Frontend)** : `cd frontend-web && npm run dev` (Écoute sur le port 5173)

#### 2.4. Orchestration locale avec Docker (Alternative)

Pour garantir une stricte parité entre les environnements de développement et éviter les problèmes liés aux versions de Node.js, la plateforme (Backend + Frontend Web) peut être exécutée via des conteneurs Docker.

##### Prérequis supplémentaires

-   Docker Engine et Docker Compose installés et en cours d'exécution sur votre machine.

##### Exécution via Docker Compose

Assurez-vous que vos fichiers `.env` sont bien configurés. Ensuite, depuis la racine du monorepo (où se trouve le fichier `docker-compose.yml`), lancez la commande suivante pour construire et démarrer les conteneurs en arrière-plan :

```bash
docker-compose up --build -d
```

##### Accès aux services et gestion

Une fois les conteneurs montés, les services sont accessibles sur leurs ports respectifs :

-   **Backend API** : `http://localhost:3000`
-   **Frontend Web** : `http://localhost:5173`

Pour stopper et détruire les conteneurs proprement une fois le développement terminé :
...
> **Note** : L'utilisation du flag `--legacy-peer-deps` est indispensable pour garantir la stabilité de l'environnement de test face aux exigences strictes de React 19.
...
(Préfixe obligatoire : `EXPO_PUBLIC_`)
```dotenv
...
npx expo start
```
**Action** : Scannez le QR Code affiché dans le terminal avec l'application Expo Go, ou appuyez sur la touche `a` (Android) / `i` (iOS) pour utiliser un émulateur.
...
-   **Backend (Render)** : À chaque mise à jour sur la branche `main`, Render intercepte le code, compile le TypeScript (`npm run build`) et redémarre le serveur (`npm run start:prod`).
-   **Frontend Web (Vercel)** : Vercel détecte les changements, exécute le build, et déploie le dossier `dist/` de manière distribuée.
-   **Mobile (EAS Build)** : La compilation des exécutables est gérée dans le cloud par Expo Application Services.
